...
function validateUser(values: { email: string, password: string }) {
  ...
  if (values.password.length < 9 || values.password.length > 20) {
...
function validateLogin(values: userInfromation) {
  return validateUser(values);
}

function validateSignup(values: userInfromation & { passwordConfirm: string }) {
